...
export default function ItemCount({ stock, initial, onAdd, contador, setContador}) {
  return (
    <div className='contador' style={{textAlign:'center', backgroundColor: "black", margin: 'auto', width: '12rem'}}>
      ...
      <button style={{backgroundColor:'whitesmoke', height: 25, width:60, alignItems: 'center'}} onClick={() =>{
         if (contador > initial){
      ...
      <button style={{backgroundColor:'whitesmoke', height: 25, width:60, textalign: 'center'}} onClick={()=>{
            if(contador < stock){
                setContador(contador + 1);
            }
        }}>+</button>
